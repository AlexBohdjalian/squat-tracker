...
def send_video(video_path):
    ...
    while True:
        ...
        draw_text(frame, 'Knee-Hip-Shoulder: ' + str(squat_details[3]), pos=(0, 100))
        draw_text(frame, 'FPS: ' + str(round(1 / (time.time() - t0), 1)), pos=(0, 130))

        cv2.imshow('MediaPipe Pose',
            resize_with_aspect_ratio(frame, height=500))
    ...
    out = cv2.VideoWriter(
        'backend/assets/processed_video.mp4',
...
if __name__ == '__main__':
    ...
    dan_squat_path = "backend/assets/dan_squat.mp4"
    
    mp_drawing = mp.solutions.drawing_utils
    mp_drawing_styles = mp.solutions.drawing_styles
    mp_pose = mp.solutions.pose       
    
    send_video(dan_squat_path)
